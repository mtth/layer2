...
(function (root) {
  ...
  function decode(buf, offset) {
    ...
    var duration = uint16_be(buf, offset + 2);
    if ((duration >> 15) & 0x01) {
    ...
    case 'ctrl':
      ...
      case 'ctrl-wrapper':
        frame.ra = unpack.macAddr(buf, offset + 4);
        frame.carriedFrame = decode(buf, offset + 16);
    ...
    case 'data':
      ...
      var isQos = (frameControl >> 7) & 0x01;
      var qosControlFieldOffset = (~flags & 0x03) ? 24 : 30;
      var isData = ~(frameControl >> 6) & 0x01;
      var isAmsduPresent = isQos & isData & (buf[qosControlFieldOffset] >> 7 & 0x01);
      ...
      var seqControl = uint16_be(buf, 22);
      frame.fragN = seqControl & 0x07;
      frame.seqN = seqControl >> 4;

      // Fill in the remaning, more tricky, addresses.
      switch (flags & 0x03) {
      case 0:
        frame.da = frame.ra;
        frame.sa = frame.ta;
      ...
      case 1: // toDs
        ...
        if (isAmsduPresent) {
          ...
          frame.da = unpack.macAddr(buf, offset + 16);
        }
        break;
      case 2: // fromDs
        frame.da = frame.ra;
        ...
        if (isAmsduPresent) {
          ...
          frame.sa = unpack.macAddr(buf, offset + 16);
        }
        break;
      default: // fromDs toDs
        if (isAmsduPresent) {
          ...
          frame.da = unpack.macAddr(buf, offset + 16);
          frame.sa = unpack.macAddr(buf, offset + 24);
        }
      }

      // And now finally, the frame's body. FIXME
      var bodyOffset = qosControlFieldOffset + (isQos ? 6 : 4);
      frame.body = new Buffer(buf.length - 4 - bodyOffset);
      buf.copy(frame.body, 0, bodyOffset, - 4);
