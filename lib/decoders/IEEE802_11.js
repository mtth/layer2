...
(function (root) {
  ...
  function Transform(opts) {
    ...
    this._transform = function (data, encoding, callback) {
      ...
      try {
        ...
        // since there are corrupted frames relatively often.
        return callback();
      }
  ...
  function decode(buf) {
    ...
    if (actualFcs !== computedFcs) {
      throw new Error(
        'Incorrect FCS: 0x' + actualFcs.toString(16) +
        ' != 0x' + computedFcs.toString(16)
      );
    }
        ...
        if (hasData) {
          ...
          if (!isProtected) {
            ...
            if ((buf[encryptionOffset + 3] >>> 5) & 0x01) {
              ...
              frame.encryption = buf[encryptionOffset + 2] ? 'tkip' : 'ccmp';
