...
(function (root) {
   ...
   * parallel batch calls [default: `1`].
   *
  ...
  function BatchReadable(fn, opts) {
    ...
    this._read = function () {
      ...
      if (!iters.length && !finishing) {
        // `elem` must be null too. We're out of things, we wait for the next
        // batch to arrive.
        needPush = true;
      } else {
        // `elem` is null if finishing and not otherwise. We push it in both
        // cases!
        this.push(elem);
      }
