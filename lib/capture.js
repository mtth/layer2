...
(function (root) {
  ...
  function Capture(wrapper, buffer, opts) {
    ...
    stream.Duplex.call(this, {
      ...
      // object mode streams. 1 is sufficient to have each read trigger the
      // next push.
    });
    ...
    this.on('finish', function () {
      // Not listening to end since the readable side doesn't seem to end when
      // the writable does (e.g. the end event isn't triggered by the end
      // method).
  ...
  util.inherits(Capture, stream.Duplex);
  ...
  Replay.prototype._write = function () {
