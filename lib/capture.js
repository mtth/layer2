...
(function (root) {
  ...
  function Capture(reader, buffer, opts) {
    ...
    if (reader.getMaxPacketSize() > buffer.length) {
    ...
    this.getMaxPacketSize = function () { return reader.getMaxPacketSize(); };
  ...
  function Live(dev, opts) {
    ...
    this.getStats = function () { return reader.getStats(); };
  ...
  function Save(path, datalink, opts) {
    ...
    var writer = new pcap.Writer(path)
      .fromOptions(datalink, maxPacketSize);
