...
Handle<Value> PcapReader::init(const Arguments& args) {
...
Handle<Value> PcapReader::activate(const Arguments& args) {
...
Handle<Value> PcapReader::dispatch(const Arguments& args) {
...
Handle<Value> PcapReader::close(const Arguments& args) {
...
Handle<Value> PcapReader::inject(const Arguments& args) {
  ...
  if (pcap_inject(pcap->handle, bufferData, bufferLength) != (int)bufferLength) {
      return ThrowException(Exception::Error(String::New("PcapReader inject failed.")));
  }
...
void PcapReader::onPacket(
  ...
  HandleScope scope;
  PcapReader *pcap = (PcapReader *) reader_p;
...
void PcapReader::expose(Handle<Object> exports) {

  char className[] = "Reader";
